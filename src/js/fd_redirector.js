								...
								if (my_self.selectedNodeInfo["FD"] != "undefined") {
									...
									input_file_dom.change(function(evt) {
										var file_path = "" + $(this).val();
										 // Saco la ultima letra (modo) para que quede solo el numero del FD
										var fd = my_self.selectedNodeInfo["FD"].slice(0, -1);
										
										//Vero el modo de apertura del archivo
										var modos_posibles = {"r" : 0, "w" : 1 , "u" : 2}; 
										...
										if (file_path) {
											...
											shortcuts.gdb_request(my_self.refresh, 
									                this.debugger_id, 
									                "gdb-module-stdfd-redirect-redirect_target_to_destine_file",
									                [fd, file_path, modo]
									                );
										} else {
