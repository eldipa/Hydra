        ...
        child = exec("ps axj | awk '{printf $1 \" \"; printf $2 \" \"; print $10;}'", function (error, stdout, stderr) {
        	...
        	for(process in dict.nodes){
        		if(dict.nodes[process].ppid != "0"){
        			dict.links[i] = {"source": parseInt(process), "target": functiontofindIndexByKeyValue(dict.nodes, "pid", dict.nodes[process].ppid), "value":1};
        			i = i+1;
        		}
        	}
        ...
        .style("fill", function(d) {
            return color(d.group);
                ...
                function(d, i) {
                    ...
                    tooltip.html(my_self.graph.nodes[i].command).style("left", (d3.event.pageX) + "px").style("top",
